...
class Address
{
    ...
    Address(const char* ip, short port)
    {
        ...
        ddr.sin_addr.s_addr = inet_addr(ip);
    }
    sockaddr* GetRawAddress() const{
...
class Socket
{
    int csock;
    public:
    Socket(in type) { csock = Socket(PF_INET, type, 0); }
    ~Socket() { close(csock);}
    void Connect(const Address& addr)
    {
        int ret = connect(csock, addr.GetRawAddress(), sizeof(sockaddr_in));
    }
...
int main()
{
    ...
    char buf[128];
    int len;
    while ((len = sork.Read(buf, sizeof buf)) > 0) 
    {
        printf("%s", buf);
    }
}
...
class HTMLDownloader
{
    ...
    Address addr;
    public:
    HTMLDownloader(const char* ip) : sock(SOCK_STREAM), addr(ip, 80)
    {
    }
