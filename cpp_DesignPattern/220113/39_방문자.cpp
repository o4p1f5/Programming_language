...
// : 컨테이너의 내부 구조에 상관없이 요소에 연산을 수행하는 객체
...
// 방문자를 통해서 연산을 수행하도록 만들어주기 위해서는 아래 인터페이스를 구현해야됨
struct MenuAcceptor
{
...
class PopupMenu : public BaseMenu
{
    ...
    { 
        ...
        for(auto e : v) // 내가 소유하고 있는 모든 메뉴들에 대해서도 방문자를 전달한다.
        {
            e->Accect(visitor);
        }
    }
...
int main()
{
    ...
    menubar->Accect(&v); // 전체메뉴에서 팝업 메뉴 표현방식이 변경됩니다.
